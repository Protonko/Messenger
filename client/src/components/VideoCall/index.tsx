...
export const VideoCall: FC<IVideoCallProps> = ({calling, setCalling}) => {
  ...
  const [conversationModalVisibility, setConversationModalVisibility] =
    useState(false)
  ...
  peerConnection.current.onicecandidate = ({candidate}) => {
    if (candidate) {
      socket.emit(EventsSocket.RELAY_ICE, interlocutor, candidate)
    }
  }
  ...
  const startCall = () => {
    ...
    setCalling(false)
    interlocutor &&
      socket.emit(EventsSocket.START_CALL, interlocutor.id, account.id)
  }
  ...
  const setRemoteMediaDescription = async (
  ...
  useEffect(() => {
    ...
    socket.on(EventsSocket.DECLINE_CALL, () => {
      setConversationModalVisibility(false)
      setAlertVisibility(false)
    })

    socket.on(EventsSocket.START_CALL, (initiator: IUser) => {
  ...
  useEffect(() => {
    return () => {
      socket.off(EventsSocket.SESSION_DESCRIPTION)
    }
  }, [])
  ...
  const declineCall = () => {
    setConversationModalVisibility(false)
    setAlertVisibility(false)
    socket.emit(EventsSocket.DECLINE_CALL, interlocutor?.id)
  }
  ...
  return (
    <>
      ...
      <Modal modalVisibility={alertVisibility}>{renderCallAlert()}</Modal>
    </>
  )
}
