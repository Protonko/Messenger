...
export const createSocket = (http: ServerHttp, io: Server, app: Express) => {
  ...
  io.on(EVENTS_SOCKET.CONNECTION, (socket: Socket) => {
    ...
    socket.on(EVENTS_SOCKET.ACCEPT_CALL, (interlocutor: string, sessionDescription: RTCSessionDescriptionInit) => {
      socket.to(interlocutor).emit(EVENTS_SOCKET.ACCEPT_CALL, sessionDescription)
    })
